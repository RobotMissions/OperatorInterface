/*
 * Operator Interface Custom Example
 * ----------------------------------
 * 
 * Send the basic commands to the robot,
 * as well as custom commands for your own module.
 * 
...
void setup() {
  ...
  setModeLabel("LEDs", 2);
  setButtonLabel("Red", 0, 2);
  setButtonLabel("Orange", 1, 2);
  setButtonLabel("Yellow", 2, 2);
  setButtonLabel("Green", 3, 2);
  setButtonLabel("Blue", 4, 2);
  setButtonLabel("Purple", 5, 2);
...
void loop() {
  ...
  if(opinterface.isConnectedToRobot()) {
    ...
    // Mode 2 is out custom screen called LEDs!
    if(opinterface.getCurrentMode() == 2) {
      // go through each button
      for(int i=0; i<6; i++) {
        // check if the button is enabled
        if(opinterface.getButton(i) == 1) {
          // if it is, send the joystick data
          opinterface.joystickSimpleControl();
        }
      }
    }
